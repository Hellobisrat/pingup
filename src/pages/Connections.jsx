      ...
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='mb-8 flex flex-wrap gap-6'>
          {dataArray.map((item,index)=>(
            <div key={index} className='flex fex-col items-center justify-center
         ...
         <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
          {
            dataArray.map((tab)=>(
              <button onClick={()=>setCurrentTab(tab.label)} key={tab.label} className={`flex items-center px-3 rounded-md transition-colors
        ...
        <div className='flex flex-wrap gap-6 mt-6'>
          {dataArray.find((item)=>item.label===currentTab).value.map((user)=>(
            ...
            <div className='flex-1'>
              ...
              <p className='text-sm text-gray-600'>{user.bio.slice(0,30).slice}...</p>
